...
typedef struct {
  ...
  char *lines;   // this will act as a key pair. So for line number 1, it will
                 // determine the instructions associated with that line
  int line_count;
  ValueArray constants; // constants that're referred to in the bytecodes(like
                        // `1 + 2` can be converted to immediate `3`)
} Chunk;
